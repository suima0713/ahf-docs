# S3-Contract v1.0 PowerShellランチャー
# 運用"契約" v1.0（過実装なし）の最終実装
...
function Show-S3ContractV1Help {
    Write-Host @"
S3-Contract v1.0 PowerShellランチャー
運用"契約" v1.0（過実装なし）の最終実装

使用方法:
  .\Start-S3ContractV1.ps1 -Action <action> [parameters]

アクション:
  axis1     - ①長期EV確度
  axis2     - ②長期EV勾配（NES）
  axis3     - ③バリュエーション＋認知ギャップ（Two-Step）
  lint      - Lintで止める（③にRo40が出たらFAIL）
  summary   - 運用契約 v1.0の要約
...
function Start-S3ContractV1 {
    ...
    # Pythonスクリプト実行
    $pythonScript = "ahf/_scripts/s3_contract_v1.py"
