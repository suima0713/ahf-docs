...
使用方法:
  .\Start-S3Integrated.ps1 -Action <action> [parameters]
...
function Start-S3Integrated {
    ...
    # Pythonスクリプト実行
    $pythonScript = "ahf/_scripts/s3_integrated.py"
...
if ($Action -eq "help") {
    ...
    Start-S3Integrated -Action $Action -CardId $CardId -Hypothesis $Hypothesis -Evidence $Evidence -TestFormula $TestFormula -TTLDays $TTLDays -ImpactNotes $ImpactNotes -QQPct $QQPct -GuidanceRevisionPct $GuidanceRevisionPct -OrderBacklogPct $OrderBacklogPct -MarginChangeBps $MarginChangeBps -TestResult $TestResult
}
