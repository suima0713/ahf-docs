...
# -*- coding: utf-8 -*-
"""
AHF v0.7.2 Integrated Evaluation
T1限定・MVP-4+／Star整数＋高コントラスト
統合実行スクリプト
"""
...
def run_v072_pipeline(ticker: str) -> Dict[str, Any]:
    """
    AHF v0.7.2統合パイプラインの実行
    """
    ...
    try:
        ...
        if tri3_result.returncode == 0:
            ...
            print("TRI-3+V-Overlay 2.0算定完了")
        else:
            results["steps"].append({"step": "TRI3_V", "status": "failed", "error": tri3_result.stderr})
            print(f"TRI-3+V-Overlay 2.0算定エラー: {tri3_result.stderr}")
        
        # ④AnchorLint v1実行
        print("=== ④AnchorLint v1実行 ===")
        anchor_result = subprocess.run([
            sys.executable, "ahf_anchor_lint.py", triage_file
        ], capture_output=True, text=True, encoding='utf-8')
        
        if anchor_result.returncode == 0:
            results["steps"].append({"step": "AnchorLint", "status": "success"})
            print("AnchorLint v1完了")
        else:
            results["steps"].append({"step": "AnchorLint", "status": "failed", "error": anchor_result.stderr})
            print(f"AnchorLint v1エラー: {anchor_result.stderr}")
        
        # ⑤自動チェック実行
        print("=== ⑤自動チェック実行 ===")
        auto_checks_result = subprocess.run([
            sys.executable, "ahf_auto_checks.py", triage_file
        ], capture_output=True, text=True, encoding='utf-8')
        
        if auto_checks_result.returncode == 0:
            results["steps"].append({"step": "AutoChecks", "status": "success"})
            print("自動チェック完了")
        else:
            results["steps"].append({"step": "AutoChecks", "status": "failed", "error": auto_checks_result.stderr})
            print(f"自動チェックエラー: {auto_checks_result.stderr}")
        ...
        # 結果サマリー生成
        results["summary"] = generate_v072_summary(ticker_dir)
...
def generate_v072_summary(ticker_dir: str) -> Dict[str, Any]:
    """
    v0.7.2評価結果のサマリー生成
    """
    summary = {}
    ...
    result_files = {
        ...
        "anchor": "anchor_lint.json",
        "auto_checks": "auto_checks.json",
...
def main():
    if len(sys.argv) != 2:
        print("使用方法: python ahf_v072_integrated.py <TICKER>")
        sys.exit(1)
    
    ticker = sys.argv[1].upper()
    
    try:
        results = run_v072_pipeline(ticker)
        ...
        # 結果出力
        print("=== AHF v0.7.2 Integrated Evaluation Results ===")
        ...
        if "summary" in results and results["summary"]:
            ...
            # 自動チェック結果
            if "auto_checks" in results["summary"]:
                auto_checks = results["summary"]["auto_checks"]
                print(f"  自動チェック: α4={auto_checks['auto_checks']['alpha4_gate_pass']}, α5={auto_checks['auto_checks']['alpha5_math_pass']}, アンカー={auto_checks['auto_checks']['anchor_lint_pass']}")
        ...
        # 結果をJSONファイルに保存
        output_file = f"tickers/{ticker}/current/v072_integrated_evaluation.json"
